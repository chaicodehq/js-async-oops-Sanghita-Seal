...
export class HaveliSecurity {
  ...
  removeResident(name, passcode) {
    ...
    if (index === -1) return { success: false, message: "Resident nahi mila!" };
  ...
  verifyAccess(name) {
    ...
    if (this.#residents.find((r) => r.name === name)) {
      const log = { name, time: new Date().toISOString(), allowed: true };
      ...
      const log = { name, time: new Date().toISOString(), allowed: false };
